...
chemba = pd.DataFrame(df_list)[3:]      # chemba will contain data from the third row of the table from the data appended in df_list

for i in chemba.columns:
    chemba[i] = chemba[i].str.replace('\n\n', '')       # to replace all '\n\n' with empty space ''

LakeLevel_Chemba = pd.DataFrame()         # variable - LakeLevel_Chemba to contain the information of the table
...
LakeLevel_Chemba.to_csv('Monthly Lake Level Chembarambakkam.csv')       # csv file will read the monthly lake level of Chennai's Chembarambakkam reservoir in each year
